...
function getInstanceTypesArr() {
	return ["g3s.xlarge", "g3.4xlarge", "t3.xlarge"];
}
	...
	getInstanceTimes(function(err, data) {
		...
		async.parallel(
			[
				function(cb) {
					sendMessage(
						getPS1("Schedule-Shutdown-Notification_tpl.ps1")
							...
							.replace(/__CLOUDRIG_REMAININGMINUTES__/, data.remainingMinutes - 3),
						cb
					);
				},
				...
				function(cb) {
					sendMessage(
						getPS1("Schedule-Shutdown_tpl.ps1").replace(/__CLOUDRIG_REMAININGMINUTES__/, data.remainingMinutes - 1),
						cb
					);
				}
			],
...
function start(cb) {
	...
	var _checkSpotPrice = function(cb) {
		getAvailabilityZoneInfo(function(err, data) {
			...
			if (parseFloat(zoneInfo.SpotPrice) >= parseFloat(config.AWSMaxPrice)) {
				cb("Your max price is too low right now. Either wait for the cost to go down, or raise your max price");
				return;
			}
		...
		getBestAvailabilityZone(function(err, bestAvailabilityZone) {
			...
			reporter.report(
				"It's " +
					state.availabilityZone +
					": $" +
					bestAvailabilityZone.SpotPrice +
					" / Max price: $" +
					config.AWSMaxPrice
			);
	...
	var _request = function(cb) {
		...
		if (config.AWSInstanceType == "g2.2xlarge") {
		...
		ec2.runInstances(instanceConfig, function(err, data) {
			if (err) {
				cb(err);
				return;
			}
	...
	var _processEBSVolume = function(cb) {
		if (isCancelled) {
			cb(cancelledError);
			return;
		}
	...
	var _initialiseEphemeralDisks = function(cb) {
		if (isCancelled) {
			cb(cancelledError);
			return;
		}
		...
		if (config.AWSInstanceType == "g2.2xlarge") {
			reporter.report("Initialising Ephemeral Disks");
			sendMessage(getPS1("Initialise-Ephemeral-Disks.ps1"), cb);
		} else {
			cb();
		}
	};

	var _createShutdownNotifier = function(cb) {
		if (isCancelled) {
			cb(cancelledError);
			return;
		}
		reporter.report("Installing Shutdown notifier");
		sendMessage(getPS1("Create-Shutdown-Notification.ps1"), cb);
	};

	var _installFolders = function(cb) {
		if (isCancelled) {
			cb(cancelledError);
			return;
		}
	...
	var _initialiseDrive = function(cb) {
		if (isCancelled) {
			cb(cancelledError);
			return;
		}
	...
	var _clearPreviousScheduledTasks = function(cb) {
		if (isCancelled) {
			cb(cancelledError);
			return;
		}
		reporter.report("Clearing previous Scheduled Tasks");
		sendMessage(getPS1("Cancel-Scheduled-Shutdown.ps1"), cb);
	};

	var _createTerminationChecker = function(cb) {
		if (isCancelled) {
			cb(cancelledError);
			return;
		}
		reporter.report("Create Termination Checker");
		sendMessage(getPS1("Create-Termination-Checker.ps1"), cb);
	};

	var _addTerminationChecker = function(cb) {
		if (isCancelled) {
			cb(cancelledError);
			return;
		}
		reporter.report("Schedule Termination Checker");
		sendMessage(getPS1("Schedule-Termination-Checker.ps1"), cb);
	};

	var _configureGraphics = function(cb) {
		if (isCancelled) {
			cb(cancelledError);
			return;
		}
		reporter.report("Configuring Graphics (this can take a while)");

		sendMessage(getPS1("Configure-Graphics.ps1"), function(err, data) {
			if (err) {
				cb(err);
				return;
			}
			_reboot(function() {
				reporter.report("Setting graphics card mode");
				sendMessage(getPS1("Enable-Graphics.ps1"), cb);
			});
		});
	};
		...
		getAMI(function(err, ami) {
			...
			if (ami) {
				...
				startFunctions = [
					...
					_initialiseEphemeralDisks,

					_reboot,

					_clearPreviousScheduledTasks,

					_createShutdownNotifier,

					_createTerminationChecker,

					_scheduleReset,

					_addTerminationChecker,
